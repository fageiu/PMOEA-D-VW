...
if __name__ == '__main__':
    for project_name, version in zip(project_name_list, version_list):
        ...
        problem = ea.benchmarks.TSP1(M=2, Dim=TEST_NUM, code_graph1=_blocks_graph, timeMMS=_timeMMS)
        algorithm = ea.moea_MOEAD_VW_templet(problem, ea.Population(Encoding='P', NIND=NIND), MAXGEN=MAXGEN, logTras=1)
        dirname = "../resultRQ1/{}_{}M{}$APBC$EET{}_result".format(algorithm.name, problem.name, problem.M, project_name)
